...
const userController = {
  ...
  postUserByEmailAndPassword({ email, passwordUser }) {
    const sqlPostUserByEmailAndPassword = `
        SELECT id_user, lastname, firstname, birthdate, address, email 
        FROM users
        WHERE email = ? AND password_user = ?
    `;
    return query(sqlPostUserByEmailAndPassword, [email, passwordUser]);
  },

  async putOne(
    idUser,
    { lastname, firstname, birthdate, address, email, passwordUser }
  ) {
    const sqlPutOne = `
        UPDATE users
