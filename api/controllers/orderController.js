...
const orderController = {
    ...
    async postOne({scheduleStart, scheduleEnd, delivery, insurance, price, isPaid, idUser, idBike}){
        const sqlPostOne = `
            INSERT INTO orders (schedule_start, schedule_end, delivery, insurance, price, is_paid, id_user, id_bike) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `;
        const results =  await query(sqlPostOne, [scheduleStart, scheduleEnd, delivery, insurance, price, isPaid, idUser, idBike]);

        const insertId = results.insertId;
        return this.getById(insertId);
    },    
    
    async putOne(idOrder, {scheduleStart, scheduleEnd, delivery, insurance, price, isPaid, idUser, idBike}){
        const sqlputOne = `
            UPDATE orders
            SET schedule_start = ?, schedule_end = ?, delivery = ?, insurance = ?, price = ?, is_paid = ?, id_user = ?, id_bike = ?
            WHERE id_order = ?;
        `;
        await query(sqlputOne, [scheduleStart, scheduleEnd, delivery, insurance, price, isPaid, idUser, idBike, idOrder]);
